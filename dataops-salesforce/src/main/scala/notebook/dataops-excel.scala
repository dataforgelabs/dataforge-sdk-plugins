...
var headerDataRows = None

val incomingParam = (session.customParameters \ "HeaderRows").asOpt[Vector[JsValue]]
if(incomingParam != None){
 var headerDataRows = incomingParam.get
}
...
def HandleHeaderData2(dfWithHeader: DataFrame) : DataFrame = {
  ...
  val mappedValues = headerDataRows.map(x => getValues(x,headerMap))
  //attach the headers
